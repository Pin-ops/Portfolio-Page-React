...
class PortfolioPage extends Component {
    ...
    categoriesWithMap= portfolioData.categories.map( (item,index) =>
                                                            <h6
                                                                key={index}
                                                                className="port-cat"
    ...
    clickHandler = (parameter) => {
        parameter !== 'all' ?
            (
                this.setState( () => {
                    const newCategories = portfolioData.cards.filter ( item => item.category === parameter)
                    return {
